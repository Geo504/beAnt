...
export class TransactionEntity {
  ...
  static fromObject(object: { [key: string]: any }) {
    const { id, _id, name, value, date, type, category, status, user, account } = object;
    ...
    return new TransactionEntity(id || _id, name, value, date, type, category, status, user, account);
  }
}